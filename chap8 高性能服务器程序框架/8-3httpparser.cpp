...
//主状态机的3种状态，分别表示：当前正在分析请求行，当前正在分析头部字段，当前正在分析文本内容
enum CHECK_STATE { CHECK_STATE_REQUESTLINE = 0, CHECK_STATE_HEADER, CHECK_STATE_CONTENT };

//从状态机的三种可能状态，即行的读取状态，分别表示：读取到一个完整行、行出错和行数据尚且不完整
...
LINE_STATUS parse_line( char* buffer, int& checked_index, int& read_index )
{
	...
	//获得当前要分析的字节
		...
		//如果当前的字节是"\r"，即回车符，则说明可能读取到了一个完整行
        if ( temp == '\r' )
        {
		//如果"\r"字符碰巧是目前buffer中的最后一个已经被读入的客户端数据，那么这次分析没有读取到一个完整行
